...
//      std::cerr<<"readData() "<<bytesRead<<std::endl;
   ...
   if (m_logFile.isOpen())
   {
      if(m_logAppendCb->currentIndex()==2)
      {
         char outstr[100];
         time_t t;
         struct tm *tmp;
         ...
         l = strftime(outstr, sizeof(outstr), "%D,%T,", tmp);

         //Now iterate through the string and replace every "<CR>" with "<CR>date time,"
         for (int i=0;i<bytesRead; i++)
         {
            if (m_buf[i] > 10)
         {
               m_logFile.write(&m_buf[i], 1);
         }
            if(m_buf[i] == 13)
            {
               //If a CR is found, then write the time
               m_logFile.write(outstr, l);
               //Get rid of any other line breaks or non-printables.
               while (m_buf[i+1]<32)
               {
                  i++;
               }
      ...
      m_logFile.flush();
//      m_logFile.close();
//      m_logFile.open(QIODevice::ReadWrite | QIODevice::Append | QIODevice::Unbuffered);
   }
...
void QCPPDialogImpl::enableLogging(bool on)
{
   ...
   if (on)
   {
      ...
      if (m_logAppendCb->currentIndex()==1)
      {
         mode=mode | QIODevice::Append | QIODevice::Unbuffered;
      }
      else
      {
         mode=mode | QIODevice::Truncate | QIODevice::Unbuffered;
      }
